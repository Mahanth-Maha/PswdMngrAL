...
class PasswordManager:
    ...
    def New_Record(self):
        try:
            S = int(input("Pre Available Sites \t1.FaceBook\t2.Twitter\t3.Instagram\t4.Google\t5.Other Site\t Select a Site:"))
            if S in (1, 2, 3, 4):
                Site = self.sites[S - 1]
...
class DataBaseRecords:
    ...
    def InsertInto(self, enc_dict, username, site):
        ...
        if len(record) == 0:
            SQL = "INSERT INTO Passwords VALUES('" + username + "','" + site + "','" + enc_dict['cipher_text'] + "','" + enc_dict['salt'] + "','" + enc_dict['nonce'] + "','" + enc_dict['tag'] + "');"
            # print(SQL)
            cur.execute(SQL)
            # print("Row inserted")
            self.con.commit()
